...
class Trader:
    ...
    def voucher_trade(self, state, orders, traderObject):
        ...
        for voucher, config in self.vouchers_settings.items():
            ...
            voucher_history[voucher].append(mid)

            if len(voucher_history[voucher]) > 120:
                voucher_history[voucher] = voucher_history[voucher][-120:]

            window = config["window"]
            fair_value = int(sum(voucher_history[voucher][-window:]) / window)
            ...
            if pos < limit:
                ...
                orders.append(Order(voucher, buy_price, volume))

            # sell
            if pos > -limit:
                sell_price = fair_value + config["spread"]
                volume = min(config["volume"], pos + limit)
                orders.append(Order(voucher, sell_price, -volume))
    ...
    def record_trades(self, state, product):
    ...
    def macaron(self, orders, state, product):
        if product not in state.order_depths:
            return orders
        ...
        if observation.sunlightIndex < self.csi:
            #net long
            bid = best_ask
            volume = self.position_limit[product] - pos
            if volume > 0:
            ...
            if (self.csi_flag):
                if pos <= 0:
                    ...
                    orders, pos= self.clear_position(state, orders, product, 0)
            else:
                bid_price = int(mid_price) - 4
                ask_price = int(mid_price) + 4
                volume = 7
                if pos < self.position_limit[product]:
                    buy_volume = min(volume, self.position_limit[product] - pos)
