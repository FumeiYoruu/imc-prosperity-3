        ...
        if sell_spread > 0 and spread_z_score > self.z_score_threshold:
            ...
            if vol > 0:
                ...
                for p, weight in self.etf_components.items():
                    ...
                    comp_vol = min(comp_vol, ask_vols[p], limit)
                    if comp_vol > 0:
                        if p == "JAMS":
                            orders.append(Order(p, asks[p], comp_vol))
                            buy_order_volume[p] += comp_vol
            ...
            if vol > 0:
                ...
                for p, weight in self.etf_components.items():
                    comp_vol = vol * weight
                    limit = pos.get(p, 0) + self.position_limit[p]
                    comp_vol = min(comp_vol, bid_vols[p], limit)
                    if comp_vol > 0:
                        if p == "JAMS":
                            orders.append(Order(p, bids[p], -comp_vol))
                            sell_order_volume[p] += comp_vol
