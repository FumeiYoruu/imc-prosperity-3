...
class Trader:
    def __init__(self):
        ...
        self.vm_b = 0.01003138
        self.vm_c = 0.14760391
        ...
        # TODO: CHANGE TO 1m in actual day
        # self.timestamp_per_day = 1000000
    ...
    def run(self, state: TradingState):
        ...
        if len(self.mt_list) and state.timestamp > 0 and (state.timestamp % (self.timestamp_per_day // self.recalculation_count * 100 // 100)) == 0:
            coefficients = np.polyfit(self.mt_list, self.iv_list, deg=2)

            self.vm_a = coefficients[0]
            self.vm_b = coefficients[1]
            self.vm_c = coefficients[2]

            self.mt_list = []
            self.iv_list = []

        logger.print(f"coeffs: {self.vm_a} {self.vm_b} {self.vm_c}")
