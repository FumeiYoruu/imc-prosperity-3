...
class Trader:
    ...
    def analyze_conversion(self, state, observation, pos, thres, traderObject):
        if not observation:
            return 0
        ...
        if pos > 0:
            better_sells = [t for t in buy_trades if abs(t.price) < implied_bid - thres]
            if better_sells:
            ...
            better_buys = [t for t in sell_trades if abs(t.price) > implied_ask + thres]
            if better_buys:
    ...
    def clear_position(self, state, orders, product, thres, traderObject):
        ...
        if pos > 0:
            total_quantity = 0
            vol = 0
            better_sells = [t for t in buy_trades if abs(t.price) < best_bid - thres]
            ...
            total_quantity = 0
            vol  = 0
            better_buys = [t for t in sell_trades if abs(t.price) > best_ask + thres]
