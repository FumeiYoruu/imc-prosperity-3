...
class Trader:
    ...
    def run(self, state):
        ...
        if pos < self.position_limit:
            volume = min(self.volume, self.position_limit - pos)
            result[product].append(Order(product, buy_price, volume))

        # try to sell
        sell_price = fair_value + self.spread
        if pos > -self.position_limit:
            volume = min(self.volume, pos + self.position_limit)
            result[product].append(Order(product, sell_price, -volume))
