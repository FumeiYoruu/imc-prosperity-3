...
class Trader:
    def __init__(self):
        ...
        self.limits = {'CROISSANTS': 250, 'JAMS': 350, 'DJEMBES': 60, 'PICNIC_BASKET1': 60, 'PICNIC_BASKET2': 100}
        self.volume = 2
        self.histories = {product: [] for product in self.products}

        self.z_upper_threshold = 2
        self.z_lower_threshold = -2
        self.window = 30

    def meanRev(self, state: TradingState, product: str, limit: int):
        orders = []
        ...
        self.histories[product].append(mid_price)
        if len(self.histories[product]) <= self.window:
            return []

        def z_score(mid_price):
            recent = self.histories[product][-self.window:]
    ...
    def run(self, state: TradingState):
        ...
        for product in self.products:
            result[product] = self.meanRev(state, product, self.limits[product])
