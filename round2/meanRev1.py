...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        def z_score(mid_price, mid_prices):
        ...
        z = z_score(mid_price, self.history)
        pos = state.position.get(product, 0)
        if z < self.z_lower_threshold and pos < self.position_limit:
            ask_volume = order_depth.sell_orders[best_ask]
            volume = min(self.volume, ask_volume, self.position_limit - pos)
            if volume > 0:
                orders.append(Order(product, best_ask, volume))
        elif z > self.z_upper_threshold and pos > -self.position_limit:
